...
The classification definitions for the Trewartha classification are based on this analysis description:
...
```{r}
#| label: directories
dir_rd <- "_data/02_aggregated_data/"
dir_wr <- "_data/03_transformed_data/"
```
...
```{r}
#| label: crp_determinants

cp <- crp

cv <- "t"
fil <- paste0(cp, "_aggregated_", cv, ".parquet")
pth <- paste0(dir_rd, fil)
df <- read_parquet(pth) 
t_df <- fn_t_determinants(df)

cv <- "p"
fil <- paste0(cp, "_aggregated_", cv, ".parquet")
pth <- paste0(dir_rd, fil)
df <- read_parquet(pth) 
p_df <- fn_p_determinants(df)

d_df <- t_df %>% left_join(p_df, by = c("lon", "lat", "area", "lsm"))

crp_df <- fn_aridity_threshold(d_df)

fil <- "crp_determinants.parquet"
